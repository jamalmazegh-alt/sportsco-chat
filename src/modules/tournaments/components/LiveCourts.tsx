...
interface Props {
  ...
  /** Show next upcoming matches when no live match is found. */
          ...
          return (
            <li key={m.id}>
              <button
                ...
                className={cn(
                  "w-full flex items-center gap-3 rounded-xl border border-border bg-card px-3 py-2.5 text-left",
                ...
                <div className="flex flex-col items-center justify-center gap-0.5 w-14 shrink-0">
                  ...
                  {mode === "live" && (
                    <span className="text-[10px] font-bold uppercase tracking-wide text-rose-500">
                      {t("controlCenter.liveBadge", { defaultValue: "LIVE" })}
                    </span>
                  )}
                </div>
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  <span className="flex-1 truncate text-sm font-medium text-right">
                    {teamLabel(a)}
                  </span>
