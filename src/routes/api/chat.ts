...
- **Notifications push** : une fois l'app installée (ou même dans le navigateur sur Android/desktop), Clubero peut envoyer des notifications push natives — sans email. Une bannière propose d'activer les notifications, ou l'utilisateur peut le faire dans Réglages. ⚠️ Sur **iOS, les push ne fonctionnent QUE si l'app a été ajoutée à l'écran d'accueil** (iOS 16.4+) — depuis le navigateur Safari classique, c'est impossible. Notifications déclenchées : nouvelle convocation, rappel de convocation, réponse d'un joueur (pour les coachs), équipe complète, score saisi, nouveau message sur le mur, report ou annulation d'événement, rappel de match tournoi 30 min avant, publication d'un tirage au sort.
