...
function SeasonsTab() {
  ...
  const { data: notes = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("player_seasons").select("*").eq("player_id", playerId)).data ?? [],
  });
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select("id,title,season_label,achievement_type,status")
          .eq("player_id", playerId)
          .eq("status", "confirmed")
      ).data ?? [],
  });
          ...
          >
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm">
              ...
              {s.attendance_rate != null && (
                <span>📊 {t("journey.season.attendance", { n: s.attendance_rate })}</span>
              )}
            </div>
            {seasonAch.length > 0 && (
  ...
  return (
    <div className="space-y-2">
      <Textarea
        ...
        placeholder={t("journey.season.coachSummaryPlaceholder")}
        rows={2}
      />
      {val !== initial && (
