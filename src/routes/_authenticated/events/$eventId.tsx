...
function EventDetail() {
  ...
  const { data: event, refetch: refetchEvent } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("events")
        .select(
          "id, title, description, starts_at, ends_at, convocation_time, location, location_url, meeting_point, opponent, competition_type, competition_name, championship_id, type, status, team_id, responses_locked, convocations_sent, is_home, is_official, attachments, cancellation_reason, cancelled_at, convocation_sent_snapshot, convocation_last_sent_at, carpool_enabled, series_id, series_detached",
        )
        .eq("id", eventId)
        .single();
  ...
  async function sendConvocations() {
    ...
    const clubChannels: string[] = Array.isArray(clubChannelsRaw)
      ? (clubChannelsRaw as string[]).filter((c) => c === "email")
      : ["email"];
    const useInApp = false;
      ...
      const sendOne = (
        ...
        sendTransactionalEmail({
          ...
          templateData: {
            ...
            playerName,
            eventTitle: event.title,
            previousDate: previousStart.toISOString(),
        ...
        <DialogContent className="max-w-md">
          ...
          {teamPlayers && pickerStep === "review" && (
            <div className="space-y-3">
              ...
              {(() => {
                // Suspension check only applies to matches/tournaments, not meetings
