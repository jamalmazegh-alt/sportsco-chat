...
function Card({ teamsCount, teamsSummary, state, count, startsLabel }: {
  teamsCount: number; teamsSummary: string; state: S; count: number; startsLabel?: string;
}) {
  return (
    <div className="grid grid-cols-2 gap-2 max-w-md">
      <Link to="/" className="group relative overflow-hidden rounded-2xl border border-border bg-card p-3 hover:border-accent/50 hover:shadow-[0_0_0_3px_color-mix(in_oklab,var(--accent)_12%,transparent)]">
        ...
        <div className="relative">
          ...
          <p className="text-[11px] text-muted-foreground mt-1 uppercase tracking-wider font-semibold">Équipes</p>
          {teamsSummary && <p className="text-[10px] text-muted-foreground mt-1 truncate">{teamsSummary}</p>}
        </div>
      ...
      <Link to="/" className={cn(
        ...
        state === "live" && "border-primary shadow-[0_0_0_3px_color-mix(in_oklab,var(--primary)_15%,transparent)]",
        ...
        <div className="relative">
          <div className={cn("icon-halo h-8 w-8 rounded-lg flex items-center justify-center mb-2", state === "live" ? "bg-primary/15" : "bg-muted")}>
            <Trophy className={cn("h-4 w-4", state === "live" ? "text-primary" : "text-muted-foreground")} />
          </div>
          {state === "empty" ? (
            <>
              <p className="text-2xl font-bold leading-none text-muted-foreground font-display tracking-tight">0</p>
              <p className="text-[11px] text-muted-foreground mt-1 uppercase tracking-wider font-semibold">Tournois</p>
              <p className="text-[10px] font-semibold text-primary mt-1.5 inline-flex items-center gap-0.5">
                <Plus className="h-3 w-3" />Créer un tournoi
              </p>
            </>
          ) : (
            <>
              <p className="text-2xl font-bold leading-none font-display tracking-tight">{count}</p>
              <p className="text-[11px] text-muted-foreground mt-1 uppercase tracking-wider font-semibold">{count > 1 ? "Tournois" : "Tournoi"}</p>
              {state === "live" && (
              ...
              {state === "planned" && (
                <span className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-1.5 py-0.5 text-[9px] font-semibold text-amber-600 dark:text-amber-400">
                  <CalendarDays className="h-2.5 w-2.5" />{startsLabel ?? "À venir"}
                </span>
              )}
              {state === "done" && (
...
function PreviewWidgets() {
  return (
    <div className="min-h-screen bg-background p-8 space-y-8">
      <div>
        <h2 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-wider">État: vide</h2>
        <Card teamsCount={0} teamsSummary="" state="empty" count={0} />
      </div>
      <div>
        <h2 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-wider">État: planifié</h2>
        <Card teamsCount={3} teamsSummary="U13 · U15 · Seniors" state="planned" count={2} startsLabel="14 juil." />
      </div>
      <div>
        <h2 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-wider">État: live</h2>
        <Card teamsCount={5} teamsSummary="U11 · U13 · U15" state="live" count={1} />
      </div>
      <div>
        <h2 className="text-sm font-semibold mb-2 text-muted-foreground uppercase tracking-wider">État: terminé</h2>
