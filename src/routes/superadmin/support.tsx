...
import { Loader2, Search, User, Building2, LifeBuoy } from "lucide-react";
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl">
      <header className="mb-6">
        ...
        <h1 className="text-xl font-semibold mt-1">Support tools</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Look up a user or club, view their context, and take action. All
          lookups are recorded in the audit log.
        </p>
      </header>
      ...
      {userSummary && (
        <section className="rounded-lg border border-border bg-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            <Stat label="Team roles" value={userSummary.team_roles.length} />
            <Stat label="Player profiles" value={userSummary.player_profiles.length} />
            <Stat label="Parent links" value={userSummary.parent_of.length} />
          </div>
            ...
            <div className="pt-2">
              <Button asChild size="sm" variant="outline">
                <Link
                  ...
                  params={{ userId: userSummary.profile.id }}
                >
                  Open full user page →
                </Link>
              </Button>
      ...
      {clubSummary && (
        <section className="rounded-lg border border-border bg-card p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <Stat
              label="Plan"
              value={clubSummary.subscription?.plan ?? "—"}
            />
            <Stat
              label="Status"
              value={clubSummary.subscription?.status ?? "—"}
            />
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              {clubSummary.recent_events.slice(0, 5).map((e) => (
              ...
              {clubSummary.recent_events.length === 0 && (
          ...
          {clubSummary.club && (
            <div className="pt-2">
              <Button asChild size="sm" variant="outline">
                <Link
                  to="/superadmin/clubs/$clubId"
                  params={{ clubId: clubSummary.club.id }}
                >
                  Open full club page →
                </Link>
              </Button>
