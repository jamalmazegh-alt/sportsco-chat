...
export const searchClubMembersForAssignment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Load teams for age_group lookup (limited to this club).
    ...
    const teamById = new Map<string, { name: string; age_group: string | null }>(
      (clubTeams ?? []).map((t) => [
        t.id as string,
        { name: (t.name as string) ?? "", age_group: (t.age_group as string | null) ?? null },
      ]),
    );
    const clubTeamIds = Array.from(teamById.keys());

    // team_members for these users (players + coaches) scoped to this club's teams.
    const { data: tmRows } = clubTeamIds.length && userIds.length
      ? await supabaseAdmin
          .from("team_members")
    ...
    for (const r of tmRows ?? []) {
      ...
      const role = ((r as { role: string | null }).role ?? "").toLowerCase();
      if (!uid || !tid) continue;
      if (role === "player" || role === "joueur") {
    ...
    // Parents → children (via player_parents.parent_user_id).
    const { data: parentLinks } = userIds.length
      ? await supabaseAdmin
          .from("player_parents")
          .select("parent_user_id, player_id")
          .in("parent_user_id", userIds)
      : { data: [] as Array<{ parent_user_id: string; player_id: string }> };

    const childrenByParent = new Map<string, string[]>();
    const allChildPlayerIds = new Set<string>();
    for (const pl of parentLinks ?? []) {
      const uid = (pl as { parent_user_id: string }).parent_user_id;
      const pid = (pl as { player_id: string }).player_id;
      if (!uid || !pid) continue;
      if (!childrenByParent.has(uid)) childrenByParent.set(uid, []);
      childrenByParent.get(uid)!.push(pid);
      allChildPlayerIds.add(pid);
    }

    const childPlayerInfo = new Map<
      string,
      { name: string; team_id: string | null }
    >();
    if (allChildPlayerIds.size > 0) {
      const ids = Array.from(allChildPlayerIds);
      ...
      for (const r of childTm ?? []) {
        const pid = (r as { player_id: string | null }).player_id;
        const tid = (r as { team_id: string | null }).team_id;
        if (pid && tid && !teamByPlayer.has(pid)) teamByPlayer.set(pid, tid);
      }
      for (const p of ps ?? []) {
        const pid = (p as { id: string }).id;
        const fn = ((p as { first_name: string | null }).first_name ?? "") as string;
        const ln = ((p as { last_name: string | null }).last_name ?? "") as string;
        const initial = ln ? ` ${ln.charAt(0).toUpperCase()}.` : "";
        childPlayerInfo.set(pid, {
          name: `${fn}${initial}`.trim() || "—",
          team_id: teamByPlayer.get(pid) ?? null,
        });
      }
    }
    ...
    const rows = rawMembers
      .map((m) => {
        ...
        const rolesArr = (
          ...
          .filter(Boolean);
        const uniqRoles = Array.from(new Set(rolesArr));
        const primary = pickPrimary(uniqRoles);

        let playerCategory: string | null = null;
        let children: Array<{ name: string; category: string | null }> = [];
        let coachedTeams: Array<{ name: string; age_group: string | null }> = [];
        ...
        if (primary === "parent" || (!primary && childrenByParent.has(uid))) {
          ...
          children = pids
            .map((pid) => {
              const info = childPlayerInfo.get(pid);
              if (!info) return null;
