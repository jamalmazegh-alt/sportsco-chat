...
/**
 * Géocode d'un coup tous les lieux du club qui n'ont pas encore de coordonnées.
 *
 * Le géocodage automatique ne se déclenche qu'à l'enregistrement d'un lieu :
 * ceux créés avant restent sans coordonnées, donc sans météo, et les rouvrir un
 * par un serait fastidieux. Cette fonction sert de rattrapage, déclenchée
 * depuis l'écran d'administration des lieux.
 *
 * Best-effort : un lieu qui échoue est simplement compté comme non résolu, il
 * ne fait pas échouer les autres. Réservée aux admins du club, comme toute
 * écriture sur `club_venues`.
 */
export const backfillVenueCoordinates = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ clubId: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }): Promise<{ resolved: number; failed: number }> => {
    await assertClubRole({
      supabase: context.supabase,
      userId: context.userId,
      clubId: data.clubId,
    ...
    const { data: venues } = await context.supabase
      .from("club_venues")
    ...
    let resolved = 0;
    let failed = 0;
    // En série plutôt qu'en parallèle : les géocodeurs publics limitent le débit,
    // et un club compte quelques lieux, pas des milliers.
