...
- **Post-fail-loud (v1) :** même race → progression peut throw → `500 bracket_progression_failed` alors que la validation n'a rien persisté — message variante A **techniquement faux** dans ce race étroit (résultat *non* enregistré). Fréquence négligeable.
