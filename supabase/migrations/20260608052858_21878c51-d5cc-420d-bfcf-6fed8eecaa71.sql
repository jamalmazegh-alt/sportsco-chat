...
AS $$
  SELECT
    ...
    -- Linked player account via players.user_id, even when team_members.user_id is still NULL
    OR EXISTS (
      SELECT 1
      ...
      AND (
        ...
        OR (
          ...
          AND (
            ...
            -- Linked player account via players.user_id, even when team_members.user_id is NULL
            OR EXISTS (
              SELECT 1
              FROM public.team_members tm
              JOIN public.players p ON p.id = tm.player_id
              WHERE tm.team_id = e.team_id AND tm.role = 'player' AND p.user_id = _user_id
            )
