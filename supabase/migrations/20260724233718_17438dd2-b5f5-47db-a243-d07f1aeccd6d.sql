-- 1) Extend enum with staff_equipe (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum e
    JOIN pg_type t ON t.oid = e.enumtypid
    WHERE t.typname = 'publication_audience_type' AND e.enumlabel = 'staff_equipe'
  ) THEN
    ALTER TYPE publication_audience_type ADD VALUE 'staff_equipe';
  END IF;
END $$;

-- 2) Rebuild resolver to include staff_equipe → team coaches/dirigeants + club admins/dirigeants
...
AS $$
  ...
  users_ AS (
    ...
    -- NEW: staff_equipe → team coaches/dirigeants
    UNION
    ...
    -- NEW: staff_equipe → club admins/dirigeants (aligned with is_team_staff)
    UNION
    SELECT DISTINCT cm.user_id AS user_id
      FROM aud a
      JOIN public.teams t ON t.id = a.team_id AND t.club_id = _club_id
