CREATE OR REPLACE FUNCTION public.redeem_club_invite_v2(_token text, _mode text DEFAULT 'self'::text, _birth_date date DEFAULT NULL::date, _phone text DEFAULT NULL::text, _license text DEFAULT NULL::text, _child_first_name text DEFAULT NULL::text, _child_last_name text DEFAULT NULL::text, _child_birth_date date DEFAULT NULL::date, _child_phone text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  v_parent_phone := nullif(btrim(coalesce(_phone, '')), '');

  -- The phone typed by the signing-up adult belongs to their own profile too.
  IF v_parent_phone IS NOT NULL THEN
    UPDATE public.profiles SET phone = coalesce(phone, v_parent_phone) WHERE id = v_user;
  END IF;
  ...
  IF v_invite.team_id IS NOT NULL THEN
    IF _mode = 'child' THEN
      ...
      v_p_phone := nullif(btrim(coalesce(_child_phone, '')), '');
    ...
    ELSE
      UPDATE public.players SET
        ...
        phone = coalesce(phone, v_p_phone),
    ...
    IF _mode = 'child' THEN
      ...
      ON CONFLICT (player_id, parent_user_id) DO UPDATE
        SET phone = coalesce(public.player_parents.phone, EXCLUDED.phone);
    END IF;
