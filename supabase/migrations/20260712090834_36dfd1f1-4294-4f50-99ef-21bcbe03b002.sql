...
BEGIN
  ...
  SELECT
    ...
    -- Goals (match sheet) — no FK on these columns
    + (SELECT count(*) FROM public.event_goals
        WHERE scorer_player_id = _id OR assist_player_id = _id)
    -- Lineups (starters jsonb, bench jsonb, captain, goalkeeper) — no FK
    + (SELECT count(*) FROM public.event_lineups
        WHERE captain_player_id = _id
           OR gk_player_id = _id
           OR EXISTS (SELECT 1 FROM jsonb_array_elements(slots) e WHERE e->>'player_id' = _id::text)
           OR EXISTS (SELECT 1 FROM jsonb_array_elements(bench) e WHERE e->>'player_id' = _id::text))
  INTO v_deps;
