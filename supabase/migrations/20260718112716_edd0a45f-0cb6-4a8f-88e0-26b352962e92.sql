...
ALTER TABLE public.club_group_rules ADD CONSTRAINT club_group_rules_params_chk CHECK (
  ...
  OR (rule_type IN ('club_educators','club_staff','club_members','club_admins','club_tournament_managers')
    AND team_id IS NULL AND category IS NULL)
);

-- 2. Refresh resolve_audience_members with the new branch
...
BEGIN
  ...
  LOOP
    ...
    ELSIF sel_type = 'club_tournament_managers' THEN
      uids := uids || ARRAY(
        SELECT DISTINCT cm.user_id
        FROM public.club_members cm
        WHERE cm.club_id = _club_id
          AND (cm.roles && ARRAY['tournament_manager']::text[])
      );
      ...
      uids := uids || ARRAY(
        ...
        WHERE g.id = (sel->>'group_id')::uuid
          ...
          AND (
            ...
            OR (r.rule_type = 'club_tournament_managers' AND cm.roles && ARRAY['tournament_manager']::text[])
          )
      );
