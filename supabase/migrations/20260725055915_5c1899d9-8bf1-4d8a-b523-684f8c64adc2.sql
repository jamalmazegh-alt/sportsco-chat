...
BEGIN
  ...
  FOR _manual_id IN SELECT value::text::uuid FROM jsonb_array_elements(_manual_member_ids) LOOP
  ...
  _idx := 0;
  FOR _option_label IN SELECT btrim(value::text, '"') FROM jsonb_array_elements(_poll_options) LOOP
    IF length(btrim(_option_label)) = 0 OR length(_option_label) > 120 THEN
  ...
  _idx := 0;
  FOR _document_id IN SELECT value::text::uuid FROM jsonb_array_elements(_document_ids) LOOP
  ...
  _idx := 0;
  FOR _media_path IN SELECT btrim(value::text, '"') FROM jsonb_array_elements(_media_paths) LOOP
    IF length(btrim(_media_path)) = 0 THEN
