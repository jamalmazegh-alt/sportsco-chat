CREATE OR REPLACE FUNCTION public.redeem_club_invite_v2(_token text, _mode text DEFAULT 'self'::text, _birth_date date DEFAULT NULL::date, _phone text DEFAULT NULL::text, _license text DEFAULT NULL::text, _child_first_name text DEFAULT NULL::text, _child_last_name text DEFAULT NULL::text, _child_birth_date date DEFAULT NULL::date)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- A same token can be replayed by several client paths (login redirect,
  -- metadata replay, manual join). Once the user redeemed it as a PARENT for
  -- this team, a replay without the stashed payload defaults to 'self' and
  -- would create a player row for the adult, giving them a duplicate identity
  -- on the team. Treat that replay as a no-op.
  IF _mode <> 'child' AND v_invite.team_id IS NOT NULL AND EXISTS (
    SELECT 1
      FROM public.player_parents pp
      JOIN public.team_members tm ON tm.player_id = pp.player_id
     WHERE pp.parent_user_id = v_user
       AND tm.team_id = v_invite.team_id
  ) THEN
...
-- Clean up the duplicate adult player row created by the replayed redeem.
DELETE FROM public.team_members WHERE player_id = '07e57f4c-b169-4bf3-ab0f-97a690008b23';
DELETE FROM public.player_parents WHERE player_id = '07e57f4c-b169-4bf3-ab0f-97a690008b23';
DELETE FROM public.players WHERE id = '07e57f4c-b169-4bf3-ab0f-97a690008b23';