CREATE OR REPLACE FUNCTION public.preview_group_rule_details(_club_id uuid, _rule jsonb)
 RETURNS TABLE(user_id uuid, full_name text, email text, kind text, subtitle text, roles text[])
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF rt = 'team_players' THEN
    RETURN QUERY
      SELECT DISTINCT ON (p.id)
        ...
        coalesce(p.email, au.email::text)::text AS email,
        'player'::text AS kind,
  ...
  ELSIF rt = 'team_parents' THEN
    RETURN QUERY
      SELECT DISTINCT ON (pp.id)
        ...
        coalesce(NULLIF(trim(pp.email), ''), au.email::text)::text AS email,
        'parent'::text AS kind,
        ('Parent de ' || NULLIF(trim(concat_ws(' ', p.first_name, p.last_name)), ''))::text AS subtitle,
        ARRAY['parent']::text[]
      FROM public.team_members tm
      JOIN public.teams t ON t.id = tm.team_id
      JOIN public.players p ON p.id = tm.player_id
  ...
  ELSE
    RETURN QUERY
      ...
      SELECT
        i.user_id,
        coalesce(pr.full_name, NULLIF(trim(concat_ws(' ', pr.first_name, pr.last_name)), ''))::text AS full_name,
        au.email::text AS email,
        'member'::text AS kind,
        NULL::text AS subtitle,
        coalesce(cm.roles, CASE WHEN cm.role IS NOT NULL THEN ARRAY[cm.role::text]::text[] ELSE ARRAY[]::text[] END)
      FROM ids i
