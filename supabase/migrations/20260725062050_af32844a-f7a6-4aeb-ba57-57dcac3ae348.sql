...
ADD CONSTRAINT pub_aud_shape_chk CHECK (
  (
    audience_type = 'groupe_personnalise'
    AND group_id IS NOT NULL
    AND team_id IS NULL
    AND category_label IS NULL
    AND event_id IS NULL
  )
  OR (
    audience_type IN ('joueurs_equipe', 'parents_equipe', 'staff_equipe')
  ...
  OR (
    audience_type IN ('joueurs_categorie', 'parents_categorie')
  ...
  OR (
    audience_type IN ('joueurs_convoques', 'parents_convoques')
  ...
  OR (
    audience_type IN ('educateurs', 'dirigeants', 'selection_manuelle')
    AND group_id IS NULL
    AND team_id IS NULL
    AND category_label IS NULL
    AND event_id IS NULL
...
DECLARE
  ...
  _dispatch_row_id uuid;
  _recipients_count integer;
  _new_recipient_count integer;
  _aud jsonb;
...
BEGIN
  ...
  SELECT r.dispatch_row_id, r.recipients_count, r.new_recipient_count
  INTO _dispatch_row_id, _recipients_count, _new_recipient_count
  FROM public.publish_publication_atomic(_publication_id, 'publish', NULL) AS r
  LIMIT 1;

  IF _dispatch_row_id IS NULL THEN
    RAISE EXCEPTION 'publish_failed';
  END IF;

  RETURN QUERY SELECT
    _publication_id,
    coalesce(_dispatch_row_id, '00000000-0000-0000-0000-000000000000'::uuid),
    coalesce(_recipients_count, 0),
    coalesce(_new_recipient_count, 0);
END;
$$;
