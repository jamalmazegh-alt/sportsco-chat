...
-- Invitation de groupes / personnes aux événements de type "meeting"
-- (éducateurs, CODIR, …) avec suivi de présence — SANS toucher au
-- système de convocations joueurs (table `convocations`, réservée aux
-- players). Identité canonique = user_id. Réutilise l'audience resolver
-- existant `resolve_audience_members` + la garde `is_club_staff`.
-- Rétrocompatible : n'altère aucune table existante.
-- =========================================================

-- 1) Table de présence réunion. Statut réutilise l'enum existant
--    attendance_status ('present','absent','uncertain','pending').
CREATE TABLE public.meeting_attendees (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id uuid NOT NULL REFERENCES public.events(id) ON DELETE CASCADE,
  club_id uuid NOT NULL REFERENCES public.clubs(id) ON DELETE CASCADE,
  -- Identité canonique de la personne convoquée (dédup cross-source).
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  -- Adhésion club dérivée (nullable : un user peut ne plus être membre).
  member_id uuid REFERENCES public.club_members(id) ON DELETE SET NULL,
  -- Réponse ET présence portées par la même colonne (comme `convocations`).
  status public.attendance_status NOT NULL DEFAULT 'pending',
  -- Provenance des invitations (snapshot des sélecteurs d'audience +
  -- marqueur d'ajout manuel). Sert à l'affichage et, en LOT 2, au retrait
  -- sélectif d'un groupe sans supprimer une personne encore invitée ailleurs.
  ...
  -- Déduplication : une personne au plus une fois par réunion.
...
-- Staff du club : lecture + écriture complète (créer/retirer/pointer).
...
-- Convoqué : voit sa propre ligne …
CREATE POLICY "meeting_attendees_self_select"
  ON public.meeting_attendees FOR SELECT TO authenticated
  USING (user_id = auth.uid());

-- … et peut mettre à jour SA réponse (le server fn borne les colonnes
--    modifiées à status/comment/responded_at ; l'identité reste verrouillée).
CREATE POLICY "meeting_attendees_self_update"
  ON public.meeting_attendees FOR UPDATE TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

-- 2) updated_at auto (réutilise le trigger générique s'il existe).
...
-- 3) RPC atomique : (ré)génère la liste des convoqués d'une réunion à partir
--    d'une sélection d'audiences + de personnes ajoutées manuellement.
--
--    LOT 1 = additif et NON destructif : insère les personnes manquantes,
--    conserve telles quelles les lignes existantes (statuts / réponses déjà
--    saisis ne sont jamais écrasés). Le retrait sélectif (retirer un groupe
--    sans toucher une personne encore invitée ailleurs) + la sécurisation de
--    l'historique relèvent du LOT 2.
--
--    Résout via `resolve_audience_members` (retourne des user_id), UNION les
--    `_manual_user_ids`, filtre au club, dérive member_id, ON CONFLICT DO
--    NOTHING → idempotent (double-clic / retry sans doublon).
...
BEGIN
  ...
  -- Appelant = service_role OU le user lui-même (défense en profondeur ;
  -- la garde staff réelle est is_club_staff plus bas).
  IF NOT (
  ...
  -- Verrou de l'événement + résolution du club via l'équipe.
  ...
  -- Restriction stricte : cette mécanique n'existe QUE pour les réunions.
  ...
  -- Cible = audiences résolues UNION ajouts manuels, dédupliqués par user_id.
  WITH resolved AS (
    SELECT r.user_id
  ...
  members AS (
    -- Dérive member_id (au plus un par user au sein du club).
    SELECT t.user_id,
