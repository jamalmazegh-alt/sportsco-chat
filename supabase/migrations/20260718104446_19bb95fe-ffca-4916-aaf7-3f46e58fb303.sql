...
BEGIN
  ...
  LOOP
    ...
    IF sel_type = 'team_players' THEN
      ...
      -- Fallback : parents des joueurs sans compte (mineurs)
      uids := uids || ARRAY(
    ...
    ELSIF sel_type = 'convoked_players' THEN
      ...
      -- Fallback parents pour mineurs convoqués
      uids := uids || ARRAY(
        SELECT DISTINCT pp.parent_user_id
        FROM public.convocations c
        JOIN public.events e ON e.id = c.event_id
    ...
    ELSIF sel_type = 'club_group' THEN
      ...
      -- team_players via règle : joueurs avec compte
      uids := uids || ARRAY(
        SELECT DISTINCT p.user_id
      ...
      -- team_players via règle : fallback parents pour mineurs
      uids := uids || ARRAY(
        SELECT DISTINCT pp.parent_user_id
        FROM public.club_group_rules r
        JOIN public.club_groups g ON g.id = r.group_id
