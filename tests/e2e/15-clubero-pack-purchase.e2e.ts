...
// Hook window.location.assign so the "redirect to Stripe" navigation
// is captured rather than executed. Must be installed before page load.
  ...
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window.location, "assign", {
      ...
      value: (url: string | URL) => {
        ...
        // For external URLs we deliberately skip the real navigation so the
        // test stays on the app and we can keep asserting.
...
test.describe("Clubero pack — parcours d'achat", () => {
  let seed: SeededClub | undefined;

  test.beforeAll(async () => {
    seed = await createTestClub("pack");
  });

  test.afterAll(async () => {
    if (seed) {
      // Nettoyer l'éventuelle subscription créée pour ce test
  ...
  test("A. /pricing expose le CTA Clubero vers /register", async ({ page }) => {
    ...
    // Le titre du plan Clubero et le tarif doivent rester présents.
    await expect(page.getByText("49 €").first()).toBeVisible();

    // Tous les CTA d'inscription pointent vers /register (essai + carte
    // de plan principal). On vérifie qu'au moins un lien /register
    // existe dans la zone tarif.
    const registerLinks = page.locator('a[href="/register"]');
    expect(await registerLinks.count()).toBeGreaterThan(0);
  });

  test("B+C+D. Admin sans sub → /admin/billing → checkout Stripe → webhook → déblocage", async () => {
    if (!seed) throw new Error("seed missing");

    // Sanity: aucun abonnement actif pour ce club au départ.
...
// The block above intentionally splits the multi-step scenario into a
// dedicated test below so we can manage the Page lifecycle cleanly.
test.describe("Clubero pack — admin flow", () => {
  let seed: SeededClub | undefined;
  ...
  test("admin sans sub → /admin/billing → checkout → webhook → unlock", async ({ page }) => {
    ...
    expect(stripeUrl, `expected Stripe checkout URL, got: ${JSON.stringify(redirects)}`)
      .toBeTruthy();

    // Le row subscriptions doit déjà avoir un stripe_customer_id persisté
    // (createCheckoutSession l'upsert avant d'appeler Stripe).
    ...
    // exactement le side-effect (subscriptions.status='active' + période en
    // cours future + plan 'monthly') comme upsertSubscription le fait.
    ...
    // Confirme l'état actif côté DB via le helper RPC utilisé par l'app.
    ...
    // --- Déblocage : l'admin peut désormais atteindre /events.
    await page.goto("/events");
    await expect(page).toHaveURL(/\/events(\/|$|\?)/, { timeout: 15_000 });
    // Plus de redirection vers /admin/billing.
